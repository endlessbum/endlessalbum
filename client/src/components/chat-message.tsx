...
function getAnimationClass(anim: string) {
  if (anim === "blush") return "bg-surface-hover text-text-primary px-1 rounded";
  return "bg-surface-hover text-text-primary px-1 rounded animate-pulse";
}
...
export default function ChatMessage({ message, isOwn, userId, dataTestId }: {
  ...
  function renderMessageContent() {
    ...
    if (message.type === 'ephemeral_image' || message.type === 'ephemeral_video') {
      ...
      return <div className="relative"><div className={`relative max-w-full overflow-hidden rounded-lg ${showOverlay ? 'blur-sm' : ''}`}>{message.type === 'ephemeral_image' ? (<img src={message.mediaUrl || undefined} alt="Эфемерное фото" className="w-full" />) : (<video src={message.mediaUrl || undefined} className="w-full" controls={!showOverlay} />)}</div>{showOverlay && (<button type="button" className="absolute inset-0 flex flex-col items-center justify-center bg-background text-primary-foreground text-sm gap-2 rounded-lg" onClick={() => { if (!blocked) setUnlocked(true); }}>Нажмите для просмотра</button>)}</div>;
    }
  ...
  return (
    <div className={`flex py-0.5 message-animation ${isOwn ? 'justify-end' : 'justify-start'}`} data-testid={dataTestId}>
      <div className={`flex flex-col max-w-[75%] ${isOwn ? 'items-end' : 'items-start'}`}>
        {/* bubble */}
        <div className={`rounded-2xl px-2.5 py-1.5 ${messageFontSize} ${isOwn ? 'bg-primary text-primary-foreground rounded-br-md' : 'bg-surface-hover text-text-primary rounded-bl-md'} ${hasBlushWord ? 'blush-bubble' : ''} ${isShortMessage ? 'inline-flex' : ''}`}>
          {isEditing ? (
             ...
             <DropdownMenu>
              ...
              <DropdownMenuContent align="end" sideOffset={4} className="min-w-[140px]">
                <DropdownMenuItem
                  ...
                  className="flex items-center gap-2 cursor-pointer"
                  data-testid={`${dataTestId}-reaction-action`}
                >
        ...
        {!isOwn && (
          <button
            ...
            data-testid={`${dataTestId}-reaction-heart`}
            onClick={toggleHeartReaction}
            disabled={reactionMutation.isPending}
          >
            <span>❤️</span>
      ...
      {deleteOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background">
          <div className="bg-popover rounded-2xl shadow-xl p-4 min-w-[280px] max-w-[90vw] flex flex-col gap-3">
            ...
            <div className="flex gap-2 justify-end">
              <button className="px-3 py-1.5 rounded-lg hover:bg-muted text-sm" onClick={() => setDeleteOpen(false)} disabled={loading}>Отмена</button>
              <button className="px-3 py-1.5 rounded-lg bg-destructive text-destructive-foreground text-sm" onClick={handleDelete} disabled={loading}>Удалить</button>
            </div>
          </div>
        </div>
      )}
