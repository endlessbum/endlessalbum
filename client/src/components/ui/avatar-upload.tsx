...
export function AvatarUpload({ currentAvatarUrl, username, onAvatarChange }: AvatarUploadProps) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      if (!response.ok) {
        // Читаем тело один раз: после .json() оно израсходовано, поэтому
        // повторный .text() вернул бы пустую строку и потерял бы сообщение
        // об ошибке от сервера.
        ...
        throw new Error(errorData?.message || errorData?.error || text || 'Failed to upload avatar');
      }
